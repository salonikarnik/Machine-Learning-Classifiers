...
for(i in 1:10) {
  ...
  library ("e1071")
  svmModel <- svm(trainingData[Class]~., data = training_data,cost = 100,gamma=1000) 
  SVMprediction <- predict(svmModel, test_data[,-1])
  conf <- table(pred = SVMprediction, true = test_data[,1])
  ...
  method="Neural Net"
  library("neuralnet")
  netModel <- neuralnet(trainingData[Class] ~gre + gpa+ rank, data= trainingData ,hidden = 4, lifesign = "minimal",linear.output = FALSE, threshold = 0.1)
  temp_test <- subset(test_data, select = c("gre", "gpa","rank"))
